...
When using AI chat features on server-rendered routes like task detail pages, ensure the UI is rendered within the <code>ChatProvider</code> boundary. This prevents hydration errors and ensures proper context control for hooks like <code>useAiChatContext</code>.
