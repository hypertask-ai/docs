...
AI responses now follow shared house-style rules in both AI Chat and custom instructions to reduce "AI tells" and unnatural phrase patterns. ADHD-shaping feedback applies only to chat-level refinements â\x80\x94 modes like Archive and Summarize do not append content to your text.
...
- Press Ctrl+K: every action is one keystroke away â\x80\x94 Tap the "New task" action, then the AI Task Writer to create a task with natural-language instructions.
- Ask the AI to build this board for you â\x80\x94 Paste a description like "a board for lighthearted weekly recaps with a backlog, doing, and done column" and let the AI draft the structure.
- Run tasks/tools â\x80\x94 Create a task that references will-be-created tasks or external actions via the Task and Tool options from within the AI Task Writer.
...
- **Gauge selector** â\x80\x94 A compact horizontal gauge shows four top models with their names, enabling quick selection without a full list.
- **Context chips** â\x80\x94 Each chip highlights the selected model and shows prior actions, keeping the model identity visible at a glance.
- **Canonical radius** â\x80\x94 Menu rows use the design-system 4px radius for consistent styling.
...
- **/skill** â\x80\x94 Open the skills menu to tap a skill by name or first letter.
- **/skill `name`** â\x80\x94 Run a specific skill if you know its name.

Known skills include:
- **Defer** â\x80\x94 Suggest a deferral with a new due date and optional sub-task.
- **Duplicate** â\x80\x94 Mirror the current task into a draft that you can rename and refine.
- **Subtask** â\x80\x94 Generate a new subtask rooted from the current one.
...
For complex tickets â\x80\x94 new features, 3+ files, schema/auth/billing changes, new subsystems, or proposals with open design choices â\x80\x94 agents now run a plan-grill step. The agent writes a numbered plan, then grills it once through an adversarial Codex review. This adversarial check surfaces misalignment and risks before implementation, improving stability for major tickets.
...
- "What's on my My Tasks board?" â\x80\x94 Generates a concise list of assigned tasks with priorities and due dates.
- "Any overdue or blocked work?" â\x80\x94 Flags urgent or late items for follow-up.
- "Show me tasks due this week" â\x80\x94 Filters and presents upcoming commitments.
...
An AI agent can update task metadata in a visible way â\x80\x94 priority, assignees, labels, due dates, or related tasks. When an agent makes a structural or status change, an AI status update appears on the task card so you see the change instantly without opening every detail.
...
- **View filtered time reports** â\x80\x94 Ask AI Chat to show time entries for specific projects, tasks, or date ranges.
- **Update shared user profiles** â\x80\x94 Update profile fields like name, role, or custom attributes with correct flag handling.
- **Read complete saved reports** â\x80\x94 Pull full saved report data including filters, columns, and time ranges.
- **Delete reports with confirmation** â\x80\x94 Remove saved reports and confirm the action before it happens.
