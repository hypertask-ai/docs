...
A start date records when work is *meant to begin*, separately from the due date that records when it must be *finished*.
...
Valid values: `Daily`, `Weekdays`, `Weekly`, `Biweekly`, `Monthly`, or `null` to stop repeating. Requires membership of the task's board.
