...
- `GET /api/pages/versions` — lists all versions with timestamps and author.
- `POST /api/pages/restore` — restores a page to a chosen version.
...
- `GET /api/tasks/description-versions` — list the snapshots of a task description.
- `POST /api/tasks/description-restore` — restore one of them.
...
| `hypertask pages update <page-id> [--title "title"] [--content "..."] [--mode replace, append, or prepend]` | Rename a page, rewrite its content, or both. |
...
Pages run full-width on desktop with a thin 12px gutter inside the editor when the app rail is visible, and 8px of vertical space above the utility row. The constrained 900px document card layout and its padding are no longer present, so full-width writing is the default experience.

<Aside type="note">
On mobile, pages have the same full-width layout as desktop, with the app rail collapsed to preserve screen space.
</Aside>
