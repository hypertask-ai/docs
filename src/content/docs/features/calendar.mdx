...
Open **Settings → Calendar** and select **Connect** under Google Calendar. After you approve access with Google, Hypertask creates a separate calendar named **Hypertask** and keeps tasks assigned to you with due dates updated there.

Each task appears as a one-hour event with a link back to Hypertask. Changing its title, due date, board, or status updates the Google Calendar event during the next sync.

Turn off **Keep tasks updated** to remove Hypertask-managed events without touching personal events. Select **Disconnect** to remove managed events and revoke Hypertask's Google access. The Hypertask calendar remains in your Google account so any personal events added during cleanup are always preserved.
...
On mobile agenda cards, blocked tasks now show the person blocking them with a red chip. This chip appears on both blocked and unblocked calendar agenda rows, so you can always see who's holding up work at a glance.
