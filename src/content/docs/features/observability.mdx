...
Hypertask routes analytics through the first-party domain <code class="language-bash">e.hypertask.ai</code>. Client-side tracking sends requests to the proxy instead of directly to PostHog servers.

**Effects:**
- Zero requests to posthog.com reach the browser
- Proxy handles CORS preflight, asset loads, and feature flags
- Analytics capture returns <code>Ok</code> on all tracked events
