...
| Links to existing tickets | No | No (drafts only) | Yes — auto-watches threads |
| Team-scoped | No (workspace fallback token) | Single-tenant | Team-scoped accounts |
| AI-driven summaries | No | No | Yes — auto-summaries on idle cycles |
...
### Local team agent visibility in comments

Team agents appear in comments in a team-scoped way to prevent unauthorized access. Hypertask now ensures that a team agent’s name is only visible to users who have access to the task board on which the comment was posted.
...
- **Fix:** The @mention picker and poster list now use the open task’s board as the primary gate, not the most-recent board you visited. People and agents appear only when the task’s board is accessible, keeping scope consistent with team-scoping.
...
## Agent attribution in actions

AI agents can act on your tasks using move, assignment, comment, or label-change actions. Hypertask now records which agent made the change, and the activity feed and CLI always show the agent’s name instead of the account that owned the task when the bot performed the action.

### What gets attributed

Agents are attributed on write-time actions:

- Comment posts
- Task assignments (including reassignments)
- Task moves between sections
- Label changes
- Blocked-by edits

### Display in the activity feed

The task feed shows the action details including the agent:

- **Before:** Activities from agents showed the bot account name.
- **After:** Activities from bots show the agent’s name (e.g. “Cursor Dev made this comment”), making it clear who performed the work.

### CLI output

When you run `hypertask comment list --include-activity`, the history section includes the agent for each post:

- Bot-generated comments, moves, and assignments show the agent name.
- Human-created content continues to show the human account name.

### Retired agents

Agents that are retired or deleted still retain their attribution in the system. If a task was assigned by a retired agent, the history and CLI still show that agent’s name for transparency.

This makes audit trails clearer for teams that rely on agents as on-call workers, since the feed always shows the person or bot that actually acted, not just the ticket owner.
