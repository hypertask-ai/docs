...
- **Fix:** Comments now use the task board as the access gate, so the agent name is only revealed when the viewer can access that exact board.

This ensures that team-agent attribution cannot leak across boards and respects the access controls of each task.
