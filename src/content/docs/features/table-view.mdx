...
2. In the column picker, click the Ã\x82Â¯ or **edit** icon next to "Columns" to open advanced options.
3. Choose from additional columns:

**Visibility options**
- **Visible columns** Ã¢\\x80\\x94 toggle columns on and off without permanently deleting them. The table updates instantly.

**Additional columns**
- **Tags** Ã¢\\x80\\x94 render label pills from your tags/column. Shows up to 3 pills, with "+N" expanding the rest. Pill clicks jump to other tasks with the same tag.
- **Comments** Ã¢\\x80\\x94 count of comments per task with hover preview (up to 3 lines). Click to open the task.
- **Created** Ã¢\\x80\\x94 full timestamp of task creation (e.g., "Jul 26, 2026 at 10:42 AM"). Click to view original creation.
- **Column** Ã¢\\x80\\x94 which board section or column a task belongs to. Click to jump to that section on the board.
- **Status** Ã¢\\x80\\x94 copy of the board column name for card-based views (e.g., "Done", "In Progress", "Review").
- **Stale** Ã¢\\x80\\x94 indicator of task staleness when the board staleness toggle is enabled. Shows up to 7 days amber/red signal, empty when fresh, and hidden when the toggle is off. Click to sort by staleness (sorting column appears between Due and Updated).
...
- **Status column auto-shows** Ã¢\\x80\\x94 as soon as you sort, a Status column appears immediately after the Title column, matching the board column name (e.g., Done, AI Review, Bugs). The column hides again when you clear the sort.
- **Updated header right-aligned** Ã¢\\x80\\x94 the Updated header stays visibly right-aligned over the rest of the column headers.
- **Stale column option** Ã¢\\x80\\x94 when sorting by staleness, a Stale column appears between Due and Updated headers. It hides when staleness sorting is cleared or when the board staleness toggle is off.
