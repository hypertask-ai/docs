...
3. On a board table, your choices stay in the browser for that account. On <strong>My Tasks</strong>, use <strong>Columns</strong> in the toolbar (or the same command) and <strong>Save</strong> the view so the column set sticks with that My Tasks view.

My Tasks can also show <strong>Board</strong> and <strong>Labels</strong>, and labels the section field as <strong>Column</strong>. Those options are only on My Tasks.
...
On a board, column choices are per-browser. On My Tasks, save the view to keep the column set.
