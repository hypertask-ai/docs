...
- **Stale on board** — days since the task was created, regardless of activity.
- **Not stale** — the inverse: both the column age and the comment age are under the warn threshold.

All four apply the per-board staleness threshold automatically, so a filtered board always agrees with the ages shown on the cards.

Use **Not stale** to separate healthy work from work that needs attention. Note that it deliberately ignores board age: a long-lived task that is still being moved and discussed is not stale.
