...
description: "Manage cycles, planning, notifications, auto-archive, staleness detection, time tracking, and per-column auto-assign rules."
---
...
- **Project planning** — Optionally organize work into two-week cycles, set a target date, track milestones, and post the board's current health.
...
Any board member can read planning and post a health update. Only the board owner or an admin can change cycles, the target date, or milestones.
...
Turn on **Enable cycles** to add Monday-based **Current cycle** and **Next cycle** views to the existing board. Kanban remains the default view. Each cycle lasts exactly two weeks, and the planning panel shows the current start date and next date range.

Open a task and select **Cycle** in its property rail to assign it to the current or next cycle. Search by cycle number to find older cycles. Older cycles are read-only history, but you can always clear a task's cycle.

When a cycle ends, Hypertask moves its assigned, unfinished, active tasks into the next cycle automatically. It leaves completed, archived, and unassigned tasks where they are. Turning cycles off hides the cycle views and blocks new assignments without deleting cycle history or existing task assignments. Turning cycles on again starts from the current Monday without rolling stale assignments from an earlier period.
