...
Press <kbd>Ctrl</kbd> + <kbd>K</kbd> and **"Go to saved views"** to jump into a saved layout (e.g., top clusters, Important, etc.). When you open a saved view or filter the list, the keyboard and row actions apply to what's visible. This means you can use <kbd>Shift</kbd> + <kbd>E</kbd> to bulk-archive the visible rows (clusters, Important siblings, or any filter set) without opening the command palette.
...
## Mobile inbox notification nudge
...
## Inbox prefetch timing

Board data is deferred until Inbox is usable to improve startup performance. When you navigate to Inbox, the previous-board query is paused while Inbox contains placeholder data and automatically resumes after IndexedDB, network, or optimistic data is available. This ensures that on signed-in reloads, no Board request occurs before usable data, and warm-up of the previous-board query waits for stable IndexedDB and network readiness ([HTPR-5431](https://app.hypertask.ai/detail/project-15/5431)).

## Inbox prefetch and API call deduping

Each inbox load now dedupes notifications and access calls to a single round trip instead of multiple concurrent calls ([HTPR-6063](https://app.hypertask.ai/detail/project-15/6063)). This reduces redundant requests and improves load times when many notifications or assignments are present, particularly on slower network connections.

## Inbox row selection and bulk archiving

You can now select multiple rows in the Inbox and archive them in bulk using **Shift+E**. This works across filtered lists and views. The multiple selection allows you to quickly clear out large batches of notifications without opening them individually, mirroring the bulk-archive behavior in Saved views. Selected rows are persisted until you commit the action, ensuring you can review the working set before archiving ([HTPR-6206](https://app.hypertask.ai/detail/project-15/6206)).

## Undo inbox items safely

Undoing an inbox item now safely handles empty row slots. Previously, undoing an item with a missing row slot could trigger a `waitingOnSynthetic` error and crash the inbox list. When the undo path encounters a hole in the data structure, the system now skips the empty slot instead of blocking, and the inbox list remains stable after the undo completes.

### What this means for you

- **No crashes on undo**: Undoing an inbox item will never cause the page to die or throw `waitingOnSynthetic`.
- **Row holes are safe**: Empty slots in the data structure are now ignored during undo operations.
- **List stays alive**: The inbox page continues to function correctly even when undoing items with irregular row data.

If you undo an inbox item and notice it didn't revert as expected, the item may have been in a separate cluster or split that was excluded from the undo scope.

### Undo on the inbox

On narrow screens, the undo button appears on inbox rows under undoable items. Undoing an item gracefully skips empty row slots instead of crashing the list ([HTPR-6527](https://app.hypertask.ai/detail/project-15/6527)).

<Aside type="note">The undo action is available via snack bar buttons on inbox rows on mobile, or by selecting an undo action from the row's action menu on desktop.</Aside>
...
The inbox now uses stronger, more consistent bolding for the active split tab across all views. On the light theme, the active split (e.g., Important) shows clearly bold black text, while inactive tabs remain in regular weight and neutral colors. This consistency extends to all split views: Inbox, Notifications, and Saved views — ensuring the active context is immediately clear whether you're browsing from Kanban, Calendar, Table, or Inbox ([HTPR-6207](https://app.hypertask.ai/detail/project-15/6207)).

## Mobile inbox icon behaviour (HTPR-6365)

When viewing tasks on mobile, the blue remove-from-inbox control now correctly displays only for tasks that belong to your inbox. Tasks opened from the Kanban board that are not in your inbox will no longer show this icon, matching the same logic used on the Inbox screen.
...
- <kbd>Ctrl</kbd> + <kbd>A</kbd> — Select all from current split
- <kbd>Ctrl</kbd> + <kbd>X</kbd> — Select message
- <kbd>Shift</kbd> + <kbd>E</kbd> — Bulk Archive (select rows or visible set)

### J/K navigation to inbox drafts

<Aside type="tip">Press <kbd>J</kbd> and <kbd>K</kbd> across the inbox to navigate between items and <kbd>Enter</kbd> to open a draft. The navigation works across mixed content including notifications and drafts.</Aside>
