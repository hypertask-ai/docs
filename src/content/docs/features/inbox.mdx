...
Press <kbd>Ctrl</kbd> + <kbd>K</kbd> and **"Go to saved views"** to jump into a saved layout (e.g., top clusters, Important, etc.). When you open a saved view or filter the list, the keyboard and row actions apply to whatâ\x80\x99s visible. This means you can use <kbd>Shift</kbd> + <kbd>E</kbd> to bulk-archive the visible rows (clusters, Important siblings, or any filter set) without opening the command palette.
...
- The list viewport updates alongside the focused row, so the jump stays visualized and you donâ\x80\x99t lose your place in scrolling.
...
| **Dismiss** | Remove and ensure future updates donâ\x80\x99t surface |
...
This means hidden rows (e.g., notifications from tasks youâ\x80\x99ve archived, filtered-out boards, or rows censored by privacy rules) are excluded from the count and will not be archived by Inbox Zero, even if theyâ\x80\x99re currently visible in some other view.
...
If the set of visible rows changes while Inbox Zero is open, the reconfirmation prompt appears again. This protects you from accidentally committing to the wrong batch when switching between views, confirming tasks that are out of scope, or otherwise altering whatâ\x80\x99s included in the visible scope.

**Confirmation required**: After validation, Inbox Zero confirms the changes before committing, ensuring accidental bulk-action clears donâ\x80\x99t lose work. Ctrl+K â\x86x92 "Clear inbox to zero" now always shows you what it will archive and waits for your explicit confirmation before applying the changes. This prevents accidental sweeps and preserves live workflows.
...
- Preview and execution respect your current view filters; if you filter out certain notifications, they wonâ\x80\x99t appear in the preview.
...
OpenWiki is a RAG-powered notepad available on each task, supporting AI-optimized queries and your custom documentation. When you add notes to a task, theyâ\x80\x99re versioned and can be queried by agents whenever they investigate that task.
...
The inbox now uses stronger, more consistent bolding for the active split tab across all views. On the light theme, the active split (e.g., Important) shows clearly bold black text, while inactive tabs remain in regular weight and neutral colors. This consistency extends to all split views: Inbox, Notifications, and Saved views â\x80\x94 ensuring the active context is immediately clear whether youâ\x80\x99re browsing from Kanban, Calendar, Table, or Inbox ([HTPR-6207](https://app.hypertask.ai/detail/project-15/6207)).

## Mobile inbox icon behavior (HTPR-6365)
...
- <kbd>Ctrl</kbd> + <kbd>K</kbd> â\x80\x94 Open command palette with inbox options: **Snooze**, **Read/Unread**, **Mark Read**, **Clear inbox to zero**, **Go to archived inbox**
...
- <kbd>E</kbd> â\x80\x94 Archive notification
- <kbd>Enter</kbd> â\x80\x94 Open task or draft
- <kbd>J</kbd> â\x80\x94 Move focus down
- <kbd>K</kbd> â\x80\x94 Move focus up
- <kbd>Ctrl</kbd> + <kbd>A</kbd> â\x80\x94 Select all from current split
- <kbd>Ctrl</kbd> + <kbd>X</kbd> â\x80\x94 Select message
- <kbd>Shift</kbd> + <kbd>E</kbd> â\x80\x94 Bulk Archive (select rows or visible set)
