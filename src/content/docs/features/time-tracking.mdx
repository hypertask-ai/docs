...
- **Per-ticket time log modal**: Press <kbd>B</kbd> (or <kbd>Cmd+K</kbd> â\x80\x94 "Log time") on an open ticket to open an in-place dialog (day-grouped entries with a total and inline add row for any date and duration; total updates in real time as you add rows)
...
1. On an open ticket, press <kbd>B</kbd> (or <kbd>Cmd+K</kbd> â\x80\x94 "Log time")
...
<dt><kbd>Cmd+K</kbd> â\x80\x94 "Log time"</dt>
