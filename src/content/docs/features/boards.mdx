...
On narrow mobile screens, split columns on boards are constrained by a maximum height. Previously, the column container clamped at 92% of the viewport height, which could leave tasks partially off-screen or cut off before reaching the true bottom of the list.

With this update, the column container scrolls up to 98% of the viewport height, giving you enough reach to see all items in a split without scrolling beyond the document. Keyboard shortcuts and the board's detail/preview remain fully accessible. If you were working around a previously truncated view, this resolves the cutoff issue entirely and restores sticky headers when the quick-tips bar is hidden.
