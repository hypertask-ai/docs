...
- Enter the guest board directly from the root URL logged out (no redirect hop) ([HTPR-4845](https://app.hypertask.ai/detail/project-15/4845)). The guest board is seeded once and remains available in the same browser for 30 days, or until you log in or sign up.
...
- **Chat opens on load:** On desktop, AI chat opens automatically with prompts that can turn a goal into columns and tasks. The same prompts appear on signed-in Free-plan boards.
- **Connect another AI:** Free users see the existing MCP token dialog once per account. It generates or shows the token used by Claude, ChatGPT, and the Hypertask CLI, with setup instructions for each client.
- **Sticky chat:** Closing chat lasts for the current page. A later board load opens it again for guests and Free-plan boards.
- **First board persisted:** A guest board remains available in the same browser for 30 days. Logging in or signing up adopts the board into the full account.
