...
## Public intake endpoint for the Agent Template board ([HTPR-6502](https://app.hypertask.ai/detail/project-15/6502))

<span class="label label--brand">Feature</span> External agents and users can now file corrections on the Agent Template board without signing in or using an API key.

**POST https://app.hypertask.ai/api/agent-template/feedback** accepts requests to create tasks in the Agent Template board's **Backlog** section. Each submission is:

- Cleaned and size-capped (title to 80 characters, body to 20,000 characters, full request to 32 KiB).
- Limited to five requests per IP address per hour.
- Checked for pre-existing labels to minimize duplicate feedback.
...
```bash
curl -sS -X POST "https://app.hypertask.ai/api/agent-template/feedback" \
  -H "Content-Type: application/json" \
  --data '{
    "title": "Keep correction details in failed runs",
    "body": "<p>The failed run dropped the correction.</p><p>Keep it for the next run.</p>",
...
All labels must already exist on the Agent Template board before use. This endpoint is ideal for automated pipelines, scoping bots, or human users who want to report issues without navigating authentication flows.
...
Previously, switching views could leave the board persistently scrolled to the right without bringing the focused card into view. With this change, focus restoration is more predictable and matches adjacent behavior: the primary view re-positions horizontally for the focused card, and subsequent view switches follow the same rule.
...
The fix ensures that every interactive modal in the demo onboarding path receives a proper boolean toggle value instead of a non-callable placeholder. The correction eliminates crashes during the guided onboarding flow and provides a stable, incremental start to a user's first Hypertask session.
