...
<span class="label label--brand">Feature</span> Each board's ticket prefix is now deterministic based on the board title, making it easier to refer to tasks across similar boards. AI-generated task descriptions, quick toggles, and ticket lookups now use these stable prefixes instead of random identifiers.
...
Use ticket prefixes in the browser's address bar or AI prompts to reliably target tasks. The stable prefixâ€”paired with a predictable numeric suffixâ€”makes task references understandable and consistent.
...
These shortcuts are ignored while you are typing, using a modal, or holding a modifier key. They are also listed in **Settings Ã¢ÂˆÂ² Shortcuts**, the <kbd>?</kbd> shortcut panel, and the <kbd>Ctrl/Cmd+K</kbd> command palette while the beta rail is active.

## Fast task entry

When quick entry is enabled, click the **+** at the top or bottom of a Kanban column to open a compact title field in that position. Press <kbd>Enter</kbd> to save the card and keep the empty field open for the next title. Press <kbd>Esc</kbd> to close it; reopening the same field restores an unsaved title.

The existing shortcuts still open the full task creator: <kbd>C</kbd> adds at the top and <kbd>Shift+C</kbd> adds at the bottom. Use <kbd>Alt+C</kbd> for fast entry at the top or <kbd>Alt+Shift+C</kbd> for fast entry at the bottom.
...
## Board hide-empty-columns clearing on last ticket removal

Before this behavior change, when the last ticket was removed from a board that had columns with all tasks in that removed ticket, those columns disappeared. The board appeared to have no columns, even though the board still had sections that were hidden.

The system now clears the `hide-empty-columns` state for that board when the last ticket leaves. Empty columns remain visible, showing you all sections where you can drop new tickets, and the "no columns to show" help screen is not displayed because the board still has sections.

This change prevents getting stuck in a no-columns state and ensures empty columns show as a normal empty state where you can continue adding tasks.

See HTPR-6550 ([link](https://app.hypertask.ai/detail/project-15/6550)).
...
- **My Tasks** â€“ tasks assigned to you. Each member sees their own slice of the board.
- **Overdue** â€“ tasks whose due date has passed and are not in a done column.
- **Blocked** â€“ tasks blocked by another task or waiting on a person.
- **Agents** â€“ tasks assigned to an AI agent.

Built-in splits need no setup and cannot be edited or deleted; they don't appear in Manage Views. They work in both kanban and table layout, and the usual view-cycling shortcut moves through them like any other view tab. Selecting one is a per-browser choice â€“ it never changes the board's saved default view.
...
## My Tasks view controls ([HTPR-6569](https://app.hypertask.ai/detail/project-15/6569))

<span class="label label--brand">Behavior change</span> My Tasks now uses the same Save view and Reset controls as Kanban. The separate Save button is removed while saving and resetting continue to work as before.

From the **Ctrl+K** command palette, you can still use **Save view as** to create a Saved view, and **Reset view** to revert My Tasks to its default set of built-in filters. Keyboard shortcuts like <kbd>Ctrl+S</kbd> and <kbd>Ctrl+R</kbd> no longer scroll the board; they now open the Save view and Reset dialogs respectively.

This change makes view management across My Tasks and other board views more consistent.
...
### View focus restoration after switching ([HTPR-6333])

<span class="label label--brand">Behavior change</span> When you switch between views (such as My Tasks, Overdue, or Agents), the board now scrolls horizontally so that any focused card is fully visible from the left edge when possible. If the focused card would stay partially off-screen, the board moves enough to keep the card visible.

Previously, switching views could leave the board persistently scrolled to the right without bringing the focused card into view. With this change, focus restoration is more predictable and match-adjacent behavior: the primary view re-positions horizontally for the focused card, and subsequent view switches follow the same rule.
...
// legacySizing omitted â€“ uses compact tokens
...
This means you can use shortcuts like **a** (add), **e** (edit), **c** (comment), **p** (move), **m** (assign), **t** (move to inbox), and **s** (archive) on a card while staying on the boardâ€“after returning from its detail view, closing a detail modal, or clicking empty board space.
...
In-app user feedback forms now create their tickets directly on Board 15 (Project 15), section **Bugs**, labeled **user-feedback**, and priority **Urgent**â€“instead of using the retired Board 2101. Notifications to contributors are unchanged. This routing keeps feedback close to your entry point and ensures it lands on a board that supports user signal.
...
## Manage board columns panel ([HTPR-6217])

The **Manage board columns** panel in every board now correctly lists all existing columns, including columns added after the last time the panel was opened.

Previously, the panel would show an empty list or stale data if the current board's column state hadn't been refreshed.

### How it works

- **Refresh on open:** When you click the Manage board columns button, the panel loads the latest column state from the server for that board.
- **Edit safely:** Each column row remains editable, and changes (renames, reordering) are persisted when you confirm the panel.
- **Cross-board state isolation:** Columns for another board are not mixed in; each board maintains its own column set.

### How to verify

1. Open any board with several columns and note their names (e.g. Doing Ã¢ÂˆÂ² Review Ã¢ÂˆÂ² Done).
2. Click the **Manage board columns** button.
3. Confirm you see all the expected columns listed.
4. Hover on one column to reveal the pencil icon, edit its name, and save.
5. Open a second board on the same account and verify the Manage board columns panel shows only its columns.

This change ensures that managing columns is reliable regardless of how recently the board was edited or when you last opened the panel.

<span class="label label--brand">Behavior change</span> The **Manage board columns** dialog now keeps keyboard focus inside it. <kbd>Tab</kbd> and <kbd>Enter</kbd> no longer reach tasks behind the panel. Reorder, edit, visibility, add, and back controls work by keyboard, so you can manage columns without leaving the keyboard.