...
<span class="label label--brand">Behavior change</span> A newly added column is visible in **every view of the board** immediately after creation. There is no need to manually include it in Saved Views or other board views.</p>
...
## Mobile board performance

On narrow mobile screens, Boards are optimized to feel fast from the moment you open them:

- **Startup rendering:** The board renders only the first visible column immediately, then progressively renders additional columns as they approach the viewport. This reduces initial DOM size and lowers long-task spikes.
- **Secondary traffic:** Non-critical API requests (rich data, extended metadata) are deferred until the Board is usable, cutting requests and improving perceived startup speed.

These changes improve 3-second usable times on production devices, reduce max long-task duration, and result in fewer DOM elements during early rendering.
