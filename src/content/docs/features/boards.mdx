...
Every project in Hypertask has a **board** â\x80\x94 a visual workspace where tasks flow through customizable columns called **sections**. If youâ\x80\x99ve used Kanban boards before, this will feel familiar. If you havenâ\x80\x99t, think of it as a whiteboard with columns that represent stages of work.
...
If you invoke AI board creation and the model doesnâ\x80\x99t return a response after 45 seconds, the fetch cancels and an error is returned instead of leaving you in a stuck state. This prevents indefinite loading when the AI generation hangs.
...
<span class="label label--brand">Feature</span> Each boardâ\x80\x99s ticket prefix is now deterministic based on the board title, making it easier to refer to tasks across similar boards. AI-generated task descriptions, quick toggles, and ticket lookups now use these stable prefixes instead of random identifiers.
...
- Short, clean titles are compressed to predictable 3â\x80\x934 character labels (e.g. **qa-2026-08-30-ui** becomes **QAUI**). This improves readability in UI fields where space is limited.
...
Use ticket prefixes in the browserâ\x80\x99s address bar or AI prompts to reliably target tasks. The stable prefixâ\x80\x94paired with a predictable numeric suffixâ\x80\x94makes task references understandable and consistent.
...
These shortcuts are ignored while you are typing, using a modal, or holding a modifier key. They are also listed in **Settings â\x86\x92 Shortcuts**, the <kbd>?</kbd> shortcut panel, and the <kbd>Ctrl/Cmd+K</kbd> command palette while the beta rail is active.
...
Every board ships with four built-in views, on by default. A splitâ\x80\x99s tab appears in the view tab bar only while it contains at least one task, so an empty board shows none of them ([HTPR-4996](https://app.hypertask.ai/detail/project-15/4996)):
...
Saved views appear as pills in a **single row above the board**, with the boardâ\x80\x99s own name as the first pill:
...
- Drag a pill to reorder your views. The boardâ\x80\x99s own pill cannot be moved.
...
- Boards are ordered by latest activity with your current board pinned at the top:
  - The board youâ\x80\x99re currently looking at appears first.
...
The left sidebar organizes your boards into collapsible teams. When you group boards by team, they appear in the groupâ\x80\x99s accordion. Within a teamâ\x80\x99s group, the boards are sorted so that your most-used teams appear first.
...
1. The board youâ\x80\x99re currently looking at is shown first.
...
This aligns with the agents pageâ\x80\x99s activity-based sorting and ensures your most active boards are always prioritized when navigating.
...
If you log in or sign up to Hypertask and donâ\x80\x99t have any boards yet, you land on a project page that shows a real empty state instead of a blank screen. This provides clear guidance for new users and prevents the dead-end experience that previously occurred.
...
The fix ensures that every interactive modal in the demo onboarding path receives a proper boolean toggle value instead of a non-callable placeholder. The correction eliminates crash crashes during the guided onboarding flow and provides a stable, incremental start to a userâ\x80\x99s first Hypertask session.
...
Hypertaskâ\x80\x99s components now respect the compact visual scale by default, including modals. Core modals such as Task Detail, Priority, and board actions follow the themeâ\x80\x99s typography tokens at 48px/16px (header) and 44px/14px (input), with 36px/13px row heights. Min-width is set to 560px to match sibling controls (e.g. the Command Palette). Legacy-sized modals are opt-in via an explicit `legacySizing` prop.
...
This behavior matches the expectations youâ\x80\x99re used to in similar Kanban tools and keeps shortcut use smooth during rapid navigation through tasks and modals.
...
In-app user feedback forms now create their tickets directly on Board 15 (Project 15), section **Bugs**, labeled **user-feedback**, and priority **Urgent**â\x80\x94instead of using the retired Board 2101. Notifications to contributors are unchanged. This routing keeps feedback close to your entry point and ensures it lands on a board that supports user signal.
...
Legacy comment endpoints previously trusted the `botUserId` or `userId` from a client-supplied header. These are now verified against the target boardâ\x80\x99s membership and the editorâ\x80\x99s access. Attempting to post a comment or task mutation without access results in an authorization error from the server.
...
Previously, the panel would show an empty list or stale data if the current boardâ\x80\x99s column state hadnâ\x80\x99t been refreshed.
...
1. Open any board with several columns and note their names (e.g., Doing → Review → Done).
