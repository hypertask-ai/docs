...
- **Last used** — re-applies the most recently selected custom date range with a single click, mirroring the same convenience pattern as the Remind Me picker.
- **Pick a custom date** — opens a calendar so you can choose any specific date.
- **Reset** — clears the due date entirely.

The **Last used** option appears directly above the quick picks (below **Reset**), so repeating a date you've set before never requires re-opening the calendar.
