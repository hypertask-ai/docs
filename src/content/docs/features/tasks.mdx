...
The close control and back gesture prevent users from being stuck inside the modal ([HTPR-5518](https://app.hypertask.ai/detail/project-15/5518)).
...
## Auto-assign on task creation (HTPR-5488)

Auto-assign rules now run on task creation, not only when tasks are moved between columns. Tasks created directly into a column (via the bottom-of-column composer, from an inbox, or via CLI) will now respect the per-column auto-assign rule just like moved tasks do. Previously, only tasks moved into a column would be auto-assigned; tickets filed straight into the column were skipped.

## Typography consistency (HTPR-5465)

Task properties now render at the same 14px font size across the board. Previously, only the **Assignees** property rendered at 13px, making it appear out of alignment with other properties like **Workers**, **Labels**, or **Priority**. The fix updates the rendering to use the shared font size from the properties rail, ensuring all values read consistently.
