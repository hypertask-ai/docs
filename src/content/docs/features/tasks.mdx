...
All Tasks uses a fetch budget that loads rows once and then builds board splits from those rows. As a result:

- Each board sees its own split of the task list.
- Quiet boards do not lose splits in favor of busier boards.
