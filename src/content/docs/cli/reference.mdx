...
```bash
# Plain output is complete â safe to parse without --json
hypertask task get HTPR-2992
hypertask comment list HTPR-2992

# Use --json when you need structured data for tooling
...
| `2` | Invalid usage â unknown or misspelled subcommand / missing required argument |
...
<Aside type="tip">
Before v1.3.4, an unrecognised subcommand exited with code `0` and printed the root help page, making it invisible to shell scripts checking `$?`. Since v1.3.4 these exits use code `2` (the POSIX convention for command-line usage errors) and print a clear error message. Update to â¥ 1.3.4 if your scripts rely on exit codes.
</Aside>
...
```
  ...
  â  Update available: 1.3.3 â 1.3.4         â
  â  Run: npm install -g @hypertask/hypertask_cli  â
  âââââââââââââââââââââââââââââââââââââââââââââ
```
...
Dump the full CLI command and option tree as machine-readable JSON. This is the fastest way for an agent or script to enumerate every command, flag, and description in one call â no recursive `--help` grepping needed.
...
<Aside type="note">
**File attachments** are not on `task list` â use `task create --attach` or `task update --attach` to attach files. See the [Capability Discovery](#capability-discovery) section or run `hypertask capabilities --json` to see all commands that support `--attach`.
</Aside>
...
<Aside type="tip">
**Plain output is complete.** The default (non-`--json`) output of `task get` is tab-separated and contains every field â you do not need `--json` just to get the full task data. This is stated in the command's `--help` output and in `hypertask capabilities --json`. Use `--json` when you need structured output for tools like `jq`.

`task show` is an alias for `task get` â both commands are identical.
</Aside>
...
<Aside type="caution">
Earlier CLI versions exposed section creation under `hypertask project section create`. That subcommand has been removed â use `hypertask section create` instead.
</Aside>
...
<Aside type="caution">
Deleting a section removes the column from the board. Tasks inside the section are not deleted â they are left without a section and may need to be moved manually. Verify section contents before deleting.
</Aside>
...
Views are named filter/sort configurations saved in the Hypertask UI (e.g. "Dev Review & QA", "Tests Running"). The CLI exposes read-only commands to list views and fetch their pre-filtered tasks â especially useful for agents working on large boards where scanning all tasks would be noisy and slow.
...
Search for tasks by keyword. Results display full ticket titles (wrapping long titles across multiple lines in the table so nothing is cut off), links to each task, and only the columns that have data â empty columns are omitted automatically.
...
| `--json` | Output raw JSON instead of the formatted table | â |
...
<Aside type="tip">
**Plain output is complete.** The default (non-`--json`) output of `comment list` is tab-separated and contains every comment field â author, timestamp, and full text. You do not need `--json` to get all the data. This is stated in the command's `--help` output and in `hypertask capabilities --json`. Use `--json` when you need structured data for downstream tools.
</Aside>

### `comment add`

Add a comment to a task. The comment is persisted immediately and a real comment URL is returned â use `comment list` to confirm the comment appears.
...
<Aside type="tip">
**Plain `@Name` mentions work.** Writing `@Alice` in your comment text is resolved server-side â Hypertask looks up the matching project member by name and sends them a notification automatically. No need to write span markup by hand.
...
<Aside type="tip">
Use `hypertask inbox list` to see your current active notifications. Archived notifications are not shown by default, so you will need to know the IDs of the notifications you want to restore â grab them from a previous `inbox list --json` call before archiving.
</Aside>
...
# Identify read commands â their plain output is complete and tab-separated
...
# For simple reads, plain output is sufficient â no --json needed
hypertask task get HTPR-2992
hypertask comment list HTPR-2992
```
