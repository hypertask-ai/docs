...
**Output links:** In CLI 1.9.1 and later, links returned in command outputs (comments, tasks, projects, search results, context, and project create-board manifests) correctly point to the frontend host (app.hypertask.ai). This corrects a previous issue where those URLs mistakenly used the API-only host (api.hypertask.ai), making your click-through more reliable.
...
- **Error message phrasing:** "`transient challenge/rate limit — retry in a minute`" clarifies that the failure is temporary and not due to a bad token
