...
<Aside type="tip">
Before v1.3.4, an unrecognised subcommand exited with code `0`, making it invisible to shell scripts that checked `$?`. Since v1.3.4 these exits use code `2`, which is the POSIX convention for command-line usage errors. Update to â¥ 1.3.4 if your scripts rely on exit codes.
</Aside>
...
[
  ...
  },
  {
    ...
    "options": [
      { "flags": "--attach <path>", "description": "Attach a local file (repeatable)" }
    ]
  },
  {
    "name": "comment add",
...
<Aside type="tip">
**For AI agents:** `hypertask capabilities --json` is the recommended starting point for any agent that drives the CLI. All three locations where `--attach` is supported â `task create`, `task update`, and `comment add` â appear explicitly in the output, so agents do not need to infer capability from subcommand help text.
</Aside>
...
Get full details of a single task by its ticket number. Response includes the full task description, labels, `parent_id` for subtasks, and the task URL.
...
<Aside type="tip">
<code>task show</code> is an alias for <code>task get</code> â both commands are identical. The full task description is always included in the output, making <code>task get</code> the right tool to inspect a task's complete content.
</Aside>
...
# 2. Read the task details (full description included)
hypertask task get HTPR-2992
# or use the alias
