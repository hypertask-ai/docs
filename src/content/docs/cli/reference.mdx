...
<Aside type="tip">
**For AI agents:** `hypertask capabilities --json` is the recommended starting point for any agent that drives the CLI. All three locations where `--attach` is supported — `task create`, `task update`, and `comment add` — appear explicitly in the output, so agents do not need to infer capability from subcommand help text.
</Aside>
...
<Aside type="tip">
<code>task show</code> is an alias for <code>task get</code> — both commands are identical. The full task description is always included in the output, no matter how long, making <code>task get</code> the right tool to inspect a task's complete content.
</Aside>
