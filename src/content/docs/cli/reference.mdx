...
- **Error message phrasing:** "<code>transient challenge/rate limit â\x86\x94 retry in a minute</code>" clarifies that the failure is temporary and not due to a bad token
...
| `--api-url <url>` | Override the API URL (default: <code>https://api.hypertask.ai/api</code>) |
...
```bash
# Plain output is complete â\x80\x94 safe to parse without --json
hypertask task get HTPR-2992
...
```
!  Update available: 1.3.3 â\x86\x92 1.3.4         Run: npm install -g @hypertask/hypertask_cli
```
