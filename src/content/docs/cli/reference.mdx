...
<Aside type="tip">
Before v1.3.4, an unrecognised subcommand exited with code `0`, making it invisible to shell scripts that checked `$?`. Since v1.3.4 these exits use code `2`, which is the POSIX convention for command-line usage errors. Update to ≥ 1.3.4 if your scripts rely on exit codes.
</Aside>
...
<Aside type="tip">
<code>task show</code> is an alias for <code>task get</code> â both commands are identical.
</Aside>
...
```
ID             NAME
view_abc123    Dev Review & QA
view_def456    Tests Running
view_ghi789    My Open Tasks
```
