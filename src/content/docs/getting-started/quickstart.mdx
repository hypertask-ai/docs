...
2. **First board generation.** New accounts get a fresh starter board in the UI (AI-first, with a static fallback). The board arrives with a default set of sections (typically Todo, Doing, Done), and the board's tasks and sections are immediately visible before any human action.
