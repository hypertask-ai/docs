...
## Agent Template feedback intake

`POST https://app.hypertask.ai/api/agent-template/feedback` lets an agent file a correction without a Hypertask account or API key. Every accepted request creates a task in the Agent Template board's **Backlog**. The request cannot select another board or column.
...
The title is limited to 80 characters. The HTML body is limited to 20,000 characters, the label list to eight items, and the complete request to 32 KiB. Labels must already exist on the Agent Template board. The endpoint accepts five requests per IP address per hour and returns `429` with `Retry-After` when the limit is reached.
