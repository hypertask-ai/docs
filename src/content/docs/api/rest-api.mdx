...
<Aside type="note">
All requests and responses use JSON. Query parameters use `snake_case`. Some request body fields keep their existing API names, including `sectionId` and `userToAdd`; use the names shown in this reference. Task descriptions and comments accept plain text, HTML, or structural Markdown. Plain text is saved as HTML paragraphs when that capability is available to the account during staged rollout; otherwise, send HTML or structural Markdown. Drafts use HTML.
</Aside>
