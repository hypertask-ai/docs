...
<Aside type="tip">
Combine `hypertask_views_list` + `hypertask_view_get` as a first step in any agent workflow on a large board. Loading a view typically returns a focused set of 5ÃÂ¢30 tasks rather than scanning hundreds, which reduces token cost and improves response quality.
</Aside>
...
<Aside type="tip">
The CLI exposes the same data via <code>hypertask task tree &lt;ticket&gt;</code>.
</Aside>
...
| `sectionId` | number | Move to a different section (camelCase ÃÂ¢ see note below) |
...
<Aside type="caution">
**Field name casing matters.** Most update fields use `snake_case`, but the section field is `sectionId` (camelCase). Passing `section_id` (snake_case) will be silently ignored ÃÂ¢ use `sectionId` to move a task between sections.
</Aside>
...
<Aside type="tip">
**Plain `@Name` mentions are supported.** Writing `@Alice` directly in the comment `text` string is resolved server-side ÃÂ¢ the server looks up the matching project member by name and fires a notification automatically. No need to construct span markup by hand. The structured `mentions` parameter is also accepted if you already have user IDs available.
</Aside>

<Aside type="note">
**Scope:** Plain `@Name` resolution applies to comment add (this endpoint). Comment updates and task descriptions do not yet support server-side mention resolution ÃÂ¢ use the structured `mentions` parameter or the web UI for those.
</Aside>

**Example request ÃÂ¢ plain @mention in comment text:**
...
**Example request ÃÂ¢ structured mentions (when you have user IDs):**
...
<Aside type="tip">
You can also create labels on the fly by passing a new name in the <code>labels</code> array of <code>hypertask_create_task</code> or <code>hypertask_update_task</code>. Use <code>hypertask_create_label</code> when you want to pre-register labels (with a specific color) before assigning them to tasks.
</Aside>
...
<Aside type="tip">
To find user IDs, call `hypertask_list_project_members` first. For agent self-assignment, use <code>assign_self: true</code> instead of looking up the agent's own user ID.
</Aside>
...
Archive (dismiss) inbox notifications. Archived notifications are removed from the active inbox view but are not deleted ÃÂ¢ they can be restored with `hypertask_inbox_unarchive`.
