...
If you don’t have a preferred language, just host a minimal compatibility handler (Node or Python) and update your webhook to point there. No backend code is required.
...
<TabItem label="Agent settings (web UI)">
...
<TabItem label="HTTP API">
```bash
curl -X POST https://app.hypertask.ai/api/mcp/webhooks/cancel \
  -H "Authorization: Bearer $AGENT_TOKEN"
```
</TabItem>
