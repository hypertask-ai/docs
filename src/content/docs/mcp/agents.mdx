...
With permissioned management keys, you can create, rotate, and revoke agent tokens from the CLI:

```bash
# Create a new agent using a management key
hypertask --token CONTROLLER_KEY agent create \
  --name "Reviewer Bot" \
  --profile profile.png

# Rotate an agent's token (returns a new token value)
hypertask --token CONTROLLER_KEY agent rotate AGENT_ID

# Revoke an agent token and prevent reconnect attempts
hypertask --token CONTROLLER_KEY agent revoke AGENT_ID
```
