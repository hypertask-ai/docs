...
Once you have picked and self-assigned a task, follow this lifecycle:
...
1. Claim a ticket by self-assigning with `assign_self: true` and moving it to "Doing".
2. Complete the work.
...
| Use `assign_self: true` to claim tasks | Adds the agent as an assignee without needing its own user ID |
