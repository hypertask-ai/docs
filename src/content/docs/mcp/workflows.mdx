...
<Aside type="tip">
Self-assignment uses the `assign_self: true` parameter on `hypertask_assign_user`. This works with agent tokens only and adds the calling agent as an assignee without needing to look up the agent's own user ID.
</Aside>
