...
Hypertask's MCP server uses Streamable HTTP at the URL below. Every request carries its own bearer token. The server keeps no per-client session, so any instance can answer any call. A session id is ignored if a client still sends one.
...
|----------|------|-------------|
| `/mcp` | Streamable HTTP | Primary MCP endpoint. Bearer token per request. No server-side session |
