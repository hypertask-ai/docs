...
Hypertask exposes an **OpenAPI schema** that ChatGPT can consume as a custom action or GPT integration. This lets ChatGPT create tasks, list projects, search tickets, and more âˆ’ without needing native MCP support.

<Steps>

1. In ChatGPT, open **My GPTs** (or the GPT editor for a custom GPT you control).
2. Go to **Actions âˆš Add action**.
...
AI Chat now has **full read/write parity with the REST API, MCP, and CLI**. Every action available through those interfaces âˆ’ including label management, section CRUD, comment updates and deletes, moving tasks between boards, inbox unarchive, task trees, and saved views âˆ’ is also available from the chat sidebar.
...
<Aside type="tip">
For time-control features not yet available via MCP, use the <a href="/cli/reference/">Hypertask CLI</a> as a workaround âˆ’ it supports the full `hypertask time start|stop|status|running|log|report` command set against prod. The report tool resolves ticket id / unique index / task id any of which you can use as identifiers.
</Aside>

## Troubleshooting

**"Failed to Connect to MCP"** âˆ’ If your client cannot connect:
...
- **Claude Code** âˆ’ Anthropic's CLI agent
- **Claude Desktop** âˆ’ Anthropic's desktop app
- **Cursor** âˆ’ AI-powered code editor
- **Windsurf** âˆ’ Codeium's AI editor
- **ChatGPT** âˆ’ via the Hypertask OpenAPI schema (custom actions / GPTs)
- **Any MCP-compatible agent** âˆ’ anything that speaks the protocol
