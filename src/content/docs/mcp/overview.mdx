...
A limited Owner + QA rollout keeps the first tool list small. Clients with native tool deferral receive one short line per tool and load the full argument list and output shape through <code>hypertask_describe_tool</code>. Cursor and Codex can connect to <code>https://mcp.hypertask.ai/mcp?tools=meta</code> to receive only <code>hypertask_search_tools</code> and <code>hypertask_describe_tool</code>, search for a tool, and load its schema before calling it.
...
Open **Settings > MCP Servers** and add a new server. Use the <code>?tools=meta</code> URL below for a small first load (Owner + QA flag above).
...
```json
{
  "mcpServers": {
    "hypertasks": {
      "type": "http",
      "url": "https://mcp.hypertask.ai/mcp?tools=meta",
      "headers": {
        "Authorization": "Bearer YOUR_API_KEY"
      }
    }
