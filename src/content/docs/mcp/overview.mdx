...
Hypertask's MCP server uses Streamable HTTP at the URL below. Every request carries its own bearer token. A limited Owner + QA rollout answers without a server-side session. Other clients still use the existing session-backed path until that flag is released.
...
On the limited rollout, a session id is ignored if a client still sends one. Server-Sent Events are used when a long call streams progress, or as a fallback at <code>/sse</code> for older clients.
