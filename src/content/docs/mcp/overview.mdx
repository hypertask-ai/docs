...
AI Chat reads and writes: it creates and updates tasks, comments, drafts, pages, labels, views and timers, assigns people, moves tasks between boards, and archives from the inbox. It exposes 44 tools against the MCP server's 48, so the two are close but not identical.

Not in AI Chat today: skills (`hypertask_list_skills` and friends), task relations (`hypertask_link_tasks`, `hypertask_find_related_tasks`, `hypertask_next_tasks`), `hypertask_move_task_to_inbox`, and the agent-coordination tools (`hypertask_hello`, `hypertask_agent_presence`, `hypertask_board_manifest`, `hypertask_get_board_playbook`, `hypertask_decision_request`, `hypertask_task_context`).
