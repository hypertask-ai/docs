...
Hypertask's MCP server uses <strong>stateless Streamable HTTP</strong>. Every request carries its own bearer token. The server does not keep a session per client, so any instance can answer any call. No sticky load-balancer session is required.
...
A session id, if a client sends one, is ignored. Server-Sent Events are used only when a long call streams progress, or as a fallback at <code>/sse</code> for older clients.
...
|----------|------|-------------|
| `/mcp` | Stateless Streamable HTTP | Primary MCP endpoint. Bearer token per request, no server-side session |
