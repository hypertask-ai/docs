...
### Governance gates for open PRs
([HTPR-6190](https://app.hypertask.ai/detail/project-15/6190))

To prevent agents from operating on boards while developers maintain an open pull request, workers now enforce a mechanical "finish your PR first" gate. The gate reads two small marker files (`wip.lock` and `wip.tickets`) that code now writes before attempting any non-readonly work. If either file exists or references the current worker's ticket, the worker is blocked by the runtime and cannot start or continue tasks that mutate the board.

<Aside type="caution">This gate operates at the runtime level, independent of the prompt-based guidance, and will block all write operations by workers on affected tickets until the open PR is closed or the marker files are removed.</Aside>
