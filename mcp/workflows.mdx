...
When workers attempt to reserve a pickup simultaneously, the system now identifies conflicting lease agents by comparing their UUIDs. Leases carry an `agentId` field derived from the active `TaskLease` row, including both current and legacy rows. Workers retry only the conflicting lease holder, preventing livelock scenarios where multiple agents repeatedly vie for the same task. Resolve by releasing the unwanted lease or refreshing your agent's lease to obtain the exclusive ID.
